...
export LOG_FILE="/tmp/gentoo-test-config.log"
: > "${LOG_FILE}"
...
SWAP_TYPE="zram"
EXTRA_PACKAGES="app-editors/vim dev-vcs/git"
export INIT_SYSTEM FILESYSTEM HOSTNAME LOCALE BTRFS_SUBVOLUMES SWAP_TYPE EXTRA_PACKAGES

# Save
TMPFILE="/tmp/gentoo-test-config-$$.conf"
config_save "${TMPFILE}"

# Clear values
unset INIT_SYSTEM FILESYSTEM HOSTNAME LOCALE BTRFS_SUBVOLUMES SWAP_TYPE EXTRA_PACKAGES
...
assert_eq "EXTRA_PACKAGES" "app-editors/vim dev-vcs/git" "${EXTRA_PACKAGES:-}"
...
TMPFILE2="/tmp/gentoo-test-config-special-$$.conf"
