...
- **`heavy-memory.conf`** — moderate limit `-j${heavy_jobs}` (2-6 zależnie od RAM) dla Qt6/KDE (1-2 GB RAM per cc1plus, pełne `-j17` na 16-thread CPU OOM-killuje cc1plus nawet na 16 GB RAM): `dev-qt/qtbase`, `dev-qt/qtdeclarative`, `kde-frameworks/networkmanager-qt`, `kde-frameworks/kio`, `kde-frameworks/kirigami`, `kde-frameworks/ktexteditor`, `kde-plasma/libkscreen`, `kde-plasma/plasma-workspace`, `kde-plasma/plasma-desktop`, `kde-plasma/kwin`, `kde-plasma/plasma-vault`, `kde-plasma/breeze`, `kde-plasma/oxygen`, `kde-frameworks/kimageformats` (cztery ostatnie: OOM na 12 GB GPD Pocket 4)

**Build swap (`lib/swap.sh` `ensure_build_swap`)**: Skonfigurowany swap (zram/swapfile) aktywuje się dopiero na ZBOOTOWANYM systemie — build w chroocie leci z tym co ma live env. Na 12 GB Pocket 4 = 0 swapu → cc1plus OOM-killed mimo throttle. `ensure_build_swap` dolewa tymczasowy swapfile do **24 GiB total** (RAM+swap) przed fazą chroot, usuwany przez `cleanup_build_swap` po. Trzy bugi naprawione (2026-05-20): (1) próg był 8 GiB → 12 GiB maszyna pomijana; (2) `dd`+`mkswap` cicho padał na btrfs (COW file → swapon "has holes") — teraz `btrfs filesystem mkswapfile`/`chattr +C`; (3) wołany tylko w CLI `run_pre_chroot`, BRAK w TUI `_execute_chroot_phase` — dodany. Idempotentny (sprawdza `swapon --show`).
...
- **Tworzenie usera/haseł MUSI być przed kernel/desktop**: `system_create_users` (hasło root + user) była w fazie `users` PO `desktop` (najdłuższa, najbardziej awaryjna faza). Gdy desktop padał, faza `users` nigdy nie ruszała → root z `*` (zablokowany), brak usera → **system bez logowania** (SDDM nie ma kogo zalogować, SSH root blokowany przez `PermitRootLogin`). Zablokowało realny GPD Pocket 4. Fix: przeniesione do fazy `users` zaraz po `system_config`, przed `kernel`/`desktop` (install.sh `_do_chroot_phases` + kolejność w `CHECKPOINTS[]`). Dodatkowo guard: gdy `ROOT_PASSWORD_HASH` puste (np. resume bez odzyskanych haseł — hashe nie są inferowalne z systemu) → `eerror` + NIE ustawia checkpointu `users` (retry zamiast cichego lockoutu). Recovery istniejącego lockoutu: Live USB → chroot → `passwd` + `useradd`.
- **Brak checkpointu `disks` ≠ pusty dysk → blind reformat / wipe (root-cause: walidacja PRZED mountem)**: Pre-mount w `screen_progress` (`tui/progress.sh`) był bramkowany na `checkpoint_reached "disks"`. Przy resume istniejącego systemu BEZ checkpointu `disks` dysk nie był montowany → `_validate_and_clean_checkpoints` sprawdzało `${MOUNTPOINT}` (puste) → **błędnie kasowało ważne** `stage3_extract`/`portage_preconfig` → faza `disks` (`disk_execute_plan`) formatowała, a nawet bez tego `stage3_extract` re-run + jego cleanup `rm` (wszystko poza `efi`) wymazywał zbudowany system. Checkpoint `disks` znika z niewinnych powodów (glitch migracji, `checkpoint_validate` pruning, przerwany re-run, fresh-attempt padły na busy). Dwa razy o mało nie skasowało systemu (godziny buildu) na GPD Pocket 4. Fix (3 spójne): (1) `_resume_target_has_system()` w `lib/utils.sh` — read-only probe `subvol=@`/plain na `/etc/gentoo-release`; (2) pre-mount montuje też gdy `MODE=resume` i system istnieje (walidacja widzi prawdziwą zawartość → checkpointy nie kasowane); (3) faza `disks` w `tui/progress.sh` ORAZ `run_pre_chroot` w `install.sh` — gdy `MODE=resume` i system istnieje → NIE `disk_execute_plan`, tylko mount + kontynuacja. Tylko resume; fresh install bez zmian. **Uwaga: ścieżka TUI = `tui/progress.sh screen_progress`, NIE `run_pre_chroot` (to CLI) — fixy muszą iść w obie.**
