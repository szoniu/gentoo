...
- **UMPC portrait panels (GPD Pocket 4, Chuwi MiniBook X)** — auto-detekcja po DMI, automatyczny zapis `fbcon=rotate:1 video=eDP-1:panel_orientation=right_side_up` (Pocket 4) / `video=DSI-1:...` (MiniBook X) do `GRUB_CMDLINE_LINUX_DEFAULT`. Pierwszy boot (GRUB, console, SDDM/Plasma) wyświetla się poprawnie bez ręcznej konfiguracji. Cmdline przeżywa kernel updates (`grub-mkconfig` regeneruje `grub.cfg` z `/etc/default/grub`). Dla Pocket 4 dodatkowo: ALC287 Auto-Mute disable service (głośniki działają od pierwszego boota), POST-INSTALL note w `/root/POST-INSTALL-NOTES.txt` z instrukcją manualnej instalacji `gpd-fan-daemon` (brak ebuilda w main/GURU).
