...
- **Czyszczenie stanu w kreatorze: `unset`, NIGDY `=""`.** `config_save` zapisuje każdą zmienną, która jest USTAWIONA — także pustą (`[[ -n "${!var+x}" ]]`), a proces w chroocie robi `config_load` NA WIERZCH odziedziczonego środowiska. Pusty `ROOT_PARTITION` w configu skasowałby więc realne urządzenie ustalone przez `disk_execute_plan`: dracut nie dostałby `root=`, a `generate_fstab` wywołałoby `get_uuid ""` i zginęło w połowie pliku. Dla schematów auto i dual-boot te nazwy są przypisywane dopiero w `disk_execute_plan`, czyli DŁUGO po zapisie configu przez kreator — pozostawienie ich nieustawionymi jest właśnie tym, na czym opiera się przekazanie wartości przez env.
- **Stan wyboru w kreatorze przeżywa TUI_BACK**: `screen_disk_select` czyści na wejściu `ROOT_PARTITION`/`SWAP_PARTITION`/`ESP_PARTITION`/`SHRINK_*`. Bez tego powrót i wybór innego dysku zostawiał starą wartość: `disk_plan_dualboot` pomijał tworzenie partycji i formatował STARĄ (z pominięciem bramki `ERASE`, która istnieje tylko na ścieżce „use existing"), a `disk_plan_shrink` podawał numer partycji z jednego dysku do `sfdisk -N` na drugim. Niezależnie od czyszczenia `lib/disk.sh` odrzuca teraz wartości spoza `TARGET_DISK` (`_partition_to_disk` + `die`).
- **„0 zajęte" ≠ „nie udało się odczytać"**: `disk_get_partition_used_mib` zwraca niezerowy kod, gdy nie potrafi ustalić zajętości. Podłoga kreatora shrinku to `used + 1 GiB`, więc echo 0 przy nieczytelnym NTFS-ie (Fast Startup) cicho autoryzowało zjazd do 1 GiB.
- **Wolne miejsce: liczy się NAJWIĘKSZA luka, nie suma** — `disk_get_largest_free_mib`. `sfdisk --append` umieszcza partycję w JEDNEJ luce i rozciąga do następnej używanej, więc suma 15 GiB + 10 GiB przechodziła próg 20 GiB i dawała root 15 GiB.
