...
- Fingerprint → `sys-auth/fprintd` + `sys-auth/libfprint` z `USE=pam` (`install_fingerprint_tools()` w `lib/portage.sh`). **PAM konfigurowany automatycznie, tylko na systemd** (`_configure_fprintd_pam()`): najpierw próba USE-flagi `sys-auth/pambase fprintd` jeśli drzewo ją ma, inaczej wstawienie `auth sufficient pam_fprintd.so` przed pierwszym `auth … pam_unix.so` w `/etc/pam.d/system-auth` (backup `.pre-fprintd`). Bez tego kroku czytnik działa TYLKO w `fprintd-verify` — nie w GDM/SDDM ani `sudo` (Fedora robi to `authselect`em, Gentoo nie ma odpowiednika). `sufficient` a nie `required` → nieudany odcisk zawsze spada na hasło, nie da się zablokować logowania. Wpis dostaje `timeout=10 max-tries=2` (minimum modułu to 10 s, domyślnie 30; `max-tries` domyślnie 3) — pam_fprintd **blokuje** czekając na palec, więc to ogranicza, jak długo logowanie wisi, zanim spadnie na hasło. Uwaga: `system-auth` należy do `pambase` → po jego update konflikt w `etc-update`.
  **Na OpenRC PAM nie jest ruszany podczas instalacji** — nie z powodu „OpenRC nie umie", tylko dlatego, że fprintd jest aktywowany przez D-Bus, a w chroocie nie ma szyny systemowej: pytanie „czy aktywacja na tej maszynie działa?" jest tam **nieodpowiadalne**, a zła odpowiedź kosztuje zawieszone logowanie. Zamiast zgadywać, `_install_fprintd_openrc_helper()` instaluje `data/fprintd-pam-setup.sh` → `/usr/local/sbin/fprintd-pam-setup`. Skrypt uruchamiany po pierwszym boocie sprawdza kolejno: obecność `pam_fprintd.so`, działający `dbus-daemon`/`dbus-broker`, wreszcie **realną odpowiedź fprintd** na `GetDevices` przez `dbus-send` (z `timeout 20`) — i dopiero wtedy edytuje `system-auth`. Tryb `--check` = sama diagnostyka. Każde niepowodzenie kończy się komunikatem z gotowymi komendami diagnostycznymi zamiast wpisu do PAM
...
- WWAN LTE → `net-misc/modemmanager` + `net-libs/libmbim` + `net-libs/libqmi` + `sys-apps/dmidecode` (`install_wwan_tools()` w `lib/portage.sh`). Trzy rzeczy, bez których modem nie ruszy mimo zainstalowanych pakietów:
  1. **USE-flagi** — `package.use/wwan` (`net-misc/modemmanager mbim qmi`, `net-misc/networkmanager modemmanager`) pisany w `generate_make_conf()`, czyli **przed** emerge NetworkManagera (`install.sh` woła `install_network_manager` dużo wcześniej niż `install_extra_packages`); `install_wwan_tools()` ma safety net + rebuild NM `--changed-use`, gdyby flaga nie weszła. **Uwaga na kalibrację: w ::gentoo te trzy flagi są domyślnie włączone** (`+mbim +qmi` w modemmanagerze, `+modemmanager` w NetworkManagerze) — ten plik więc *pinuje*, a nie *naprawia*: chroni przed zmianą profilu albo globalnym `USE="-modemmanager"`, które po cichu zabrałyby modem z maszyny instalowanej właśnie pod niego. Osobna sprawa (i prawdziwa): emerge samych `libmbim`/`libqmi` NIE włącza wsparcia w MM — biblioteki to zależności, flagi to wpięcie
  1a. **OpenRC: `elogind`** — jedyna realna różnica init-owa dla WWAN. Bez login-managera ani MM, ani NM nie dowiadują się o suspendzie: modem zostaje w nieświeżym stanie, a NM nie wybiera połączenia po otwarciu klapy (klasyczne „WWAN działał do pierwszego uśpienia"). Na systemd załatwia to `systemd-logind` za darmo, na OpenRC trzeba `elogind` — dopisywany do `package.use/wwan` tylko przy `INIT_SYSTEM=openrc` (`REQUIRED_USE` NetworkManagera to `?? ( elogind systemd )`, więc kolizja jest niemożliwa). `install_wwan_tools()` robi też `rc-update add elogind boot`, bo usługę włączają inaczej tylko instalatory desktopów — headless OpenRC z modemem zbudowałby się z `elogind` i nigdy go nie uruchomił. Sam demon MM to na OpenRC zwykła usługa (`newinitd modemmanager` w ebuildzie), bez zgadywanki z aktywacją D-Bus jak przy fprintd
