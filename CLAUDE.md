...
- **Brak checkpointu `disks` ≠ pusty dysk → blind reformat**: `run_pre_chroot` (install.sh) gdy `! checkpoint_reached "disks"` bezwarunkowo wołał `disk_execute_plan` (repartycja + mkfs) — nawet gdy na `ROOT_PARTITION` był już wypakowany Gentoo (godziny buildu). Checkpoint `disks` potrafi zniknąć z niewinnych powodów (glitch migracji, `checkpoint_validate` pruning, przerwany re-run, fresh-attempt który padł na busy ale zostawił stary fs). Dwa razy o mało nie skasowało systemu na GPD Pocket 4. Fix: helper `_resume_target_has_system()` (read-only probe `subvol=@`/plain na `/etc/gentoo-release`) + w fazie disks gdy `MODE=resume` i system istnieje → NIE formatuje, montuje istniejące + `checkpoint_set "disks"`. Tylko resume; fresh install bez zmian.
