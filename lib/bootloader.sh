...
_configure_grub() {
    ...
    # NVIDIA (incl. hybrid dGPU — Lenovo Legion etc.): enable DRM KMS on the
    # kernel cmdline. The modprobe.d option alone is unreliable when nvidia_drm
    # autoloads before it is read (genkernel: no nvidia in initramfs) → black
    # screen / no Wayland session (SDDM/KWin). fbdev=1 gives a working fb
    # console. Belt-and-suspenders alongside /etc/modprobe.d/nvidia.conf.
    if [[ "${GPU_VENDOR:-}" == "nvidia" || "${DGPU_VENDOR:-}" == "nvidia" ]]; then
        default_params="${default_params} nvidia-drm.modeset=1 nvidia-drm.fbdev=1"
        einfo "NVIDIA DRM modeset added to GRUB_CMDLINE_LINUX_DEFAULT"
    fi
