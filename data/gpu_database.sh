#!/usr/bin/env bash
# gpu_database.sh — GPU vendor detection and NVIDIA generation database
...
nvidia_generation() {
    ...
    if (( dec_id >= 0x2900 )); then
        ...
        # threshold classified the 4090 as Ampere. Ampere tops out in the
        # 0x25xx range (GA107 = 0x25a0), so 0x2600 is a clean split.
        echo "ada"
...
# Prints: "driver_package|video_cards_value|use_open_kernel(yes/no)"
get_gpu_recommendation() {
    ...
    case "${vendor_id}" in
        "${GPU_VENDOR_NVIDIA}")
            ...
            echo "nvidia-drivers|nvidia|${use_open}"
            ;;
        "${GPU_VENDOR_AMD}")
            echo "amdgpu|amdgpu radeonsi|no"
            ;;
        "${GPU_VENDOR_INTEL}")
            echo "intel-media-driver|intel|no"
            ;;
        *)
            echo "none|fbdev|no"
            ;;
