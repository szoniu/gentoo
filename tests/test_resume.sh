...
export LOG_FILE="/tmp/gentoo-test-resume.log"
export DRY_RUN=1
...
setup_fake_partition() {
    ...
    if [[ ${has_checkpoints} -eq 1 ]]; then
        ...
        mkdir -p "${cp_dir}"
        touch "${cp_dir}/disks"
        touch "${cp_dir}/stage3_extract"
        touch "${cp_dir}/portage_preconfig"
    fi
    ...
    if [[ ${has_config} -eq 1 ]]; then
        ...
        (
            umask 077
            cat > "${fake_mp}/tmp/gentoo-installer.conf" <<'CONF'
#!/usr/bin/env bash
# Gentoo TUI Installer configuration
INIT_SYSTEM='systemd'
TARGET_DISK='/dev/sda'
FILESYSTEM='ext4'
HOSTNAME='testhost'
...
assert_eq "Checkpoint stage3_extract recovered" "true" "$([[ -f "${CHECKPOINT_DIR}/stage3_extract" ]] && echo true || echo false)"
...
assert_eq "INIT_SYSTEM loaded from config" "systemd" "${INIT_SYSTEM:-}"
